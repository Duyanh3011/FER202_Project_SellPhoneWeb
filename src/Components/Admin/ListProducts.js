...
export default function ListProducts() {
  ...
  const Pagging = (index) => {
    if (products.length > index * 10)
      ...
      setPagingProducts(
        searchProducts.slice((index - 1) * 10, products.length),
      );
    }
  };
  //Set data cho Search
  ...
  const handleFilterByCategory = (e) => {
    if (e.target.value === "all") setSearchProducts(products);
      ...
      setSearchProducts(
        products.filter((p) => p.Category_ID === e.target.value),
      );
  ...
  const handleFilterBySuppliers = (e) => {
    if (e.target.value === "all") setSearchProducts(products);
      ...
      // console.log(`ID: ${e.target.value}`)
      setSearchProducts(
        products.filter((p) => p.Supplier_ID === e.target.value),
      );
      console.log(searchProducts);
    }
  };
  ...
  const handleFilterBySearch = (e) => {
    if (e.target.value === "") setSearchProducts(products);
      ...
      setSearchProducts(
        products.filter((p) =>
          p.Name.toLowerCase().includes(search.current.value.toLowerCase()),
        ),
      );
      console.log(searchProducts);
    }
      ...
      <Container style={{ paddingTop: 50 }}>
        <Alert variant="">
          ...
          <Row style={{ marginTop: 15, marginBottom: 15 }}>
            ...
            <select
              name="suppliers"
              onChange={(e) => handleFilterBySuppliers(e)}
            >
